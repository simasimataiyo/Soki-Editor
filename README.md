...
| コマンド | 説明 |
|----------|------|
| `/structure` | ソースと執筆ルールに基づいて骨子（セクション構成）を追加・修正 |
| `/structure section` | 現在選択中のセクション配下のみ骨子を生成。既存セクションの本文は変更しない |
| `/structure replace` | 既存の骨子を全削除して骨子を再生成（確認ダイアログあり） |
| `/draft` | 選択中のセクションの概要から本文を生成 |
| `/rewrite` | 選択中のセクションの本文を書き直し |
| `/expand [文字数]` | 本文を指定文字数分加筆（例: `/expand 500`） |
| `/shorten [文字数]` | 本文を指定文字数に圧縮（例: `/shorten 300`） |
| `/tone [スタイル]` | 文体を変更（例: `/tone フォーマル`） |

コマンドの後ろに追加指示を書けます。例:

```
/draft 論点Aに重点を置いて記述してください
/expand 800 既存の文献引用を増やして補強してください
```
